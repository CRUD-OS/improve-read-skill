...
export const transcribeChimege = async (audioBase64: string): Promise<string> => {
  const CHIMEGE_API_TOKEN: string | undefined = process.env.CHIMEGE_API_TOKEN;

  if (!CHIMEGE_API_TOKEN) {
    throw new Error('CHIMEGE_API_TOKEN is not configured in environment variables.');
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Chimege API request failed with status ${response.status}: ${errorBody}`);
    }
